        ...
        {runs.map((r) => (
          <div key={r.id} className="border-b border-[#1c1c1f] py-3 last:border-0">
            <div className="console-head flex items-center gap-2.5 text-[11px]">
              ...
              <button className="cursor-pointer border-0 bg-none p-0 font-mono text-[11px] text-[var(--purple)] hover:underline" onClick={() => onOpenAgent(r.agentId)}>{agentName(r.agentId)}</button>
            ...
            {r.status === 'failed' && <div className="console-line flex items-baseline gap-2"><span className="flex-none text-[#52525b]" /><span className="w-10 flex-none text-[#22c55e]">err</span><span className="text-[#a1a1aa]">Run failed — see agent chat for details.</span></div>}
          </div>
        ))}
      </div>
    </>
