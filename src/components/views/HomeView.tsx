...
export function HomeView({ agents, tools, runs, onOpen, onCreate }: { agents: Agent[]; tools: Tool[]; runs: Run[]; onOpen: (id: string) => void; onCreate: () => void }) {
  ...
  return (
    <>
      ...
      <div className="mb-[22px] -mt-2 flex rounded-[10px] border border-line bg-panel">
        <div className="min-w-[160px] border-r border-line px-[22px] py-4">
      ...
      <div className="grid max-w-[1100px] grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-3.5">
        {agents.map((a) => (
          <button key={a.id} className="home-agent min-h-[165px] cursor-pointer rounded-[10px] border border-line bg-panel p-[18px] text-left transition-transform duration-150 hover:-translate-y-0.5 hover:border-[#71717a]" onClick={() => onOpen(a.id)} style={{ borderTop: `2px solid ${a.color}` }}>
        ...
        <div className="home-empty col-span-full flex min-h-[370px] flex-col items-center justify-center rounded-[12px] border border-dashed border-[#3f3f46] text-center text-muted">
