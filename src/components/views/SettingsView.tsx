    ...
    <div className="flex h-full flex-col">
      <header className="mb-5 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[1px] text-text">Settings</span>
        </div>
        ...
        <div className="max-w-[700px] rounded-[9px] border border-line bg-panel p-[22px]">
          ...
          <div className="mb-[31px] flex gap-[13px]">
            {(['dark', 'light', 'cyber'] as const).map((t) => (
              <button key={t} className={`w-[130px] cursor-pointer rounded-[7px] border border-line bg-transparent p-2 text-left hover:border-mid ${t === 'dark' ? 'border-[var(--green)]' : ''}`} onClick={() => document.documentElement.setAttribute('data-theme', t)}>
