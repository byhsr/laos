...
| Shell | `components/Topbar.tsx`, `components/Sidebar.tsx` | The topbar carries the workspace sections + Graph + New agent + window controls; the sidebar is the **agent chat list** — lead agent first (never deletable), then pinned, then the rest. Rows are full width with **right-click** for open chat / settings / pin / delete (delete needs the name typed); no per-row ⋯ button. Settings opens the lead's config drawer or a regular agent's Config tab. Section names are user-editable via `hooks/useNavLabels.ts`; the lead's *own* name comes from its agent record. |
