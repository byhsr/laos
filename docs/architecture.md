...
| Shell | `components/Topbar.tsx`, `components/Sidebar.tsx` | The topbar carries the workspace sections + Graph + New agent + window controls; the sidebar is the **agent chat list** — lead agent first (never deletable), then pinned, then the rest — each row with a ⋯ menu (open / pin / delete; delete needs the name typed). Section names are user-editable via `hooks/useNavLabels.ts`; the lead's *own* name comes from its agent record. |
